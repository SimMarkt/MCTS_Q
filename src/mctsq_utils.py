...
from src.mctsq_config_mcts import MCTSQConfiguration
...
def import_market_data(csvfile: str, market_type: str, path: str) -> np.ndarray:
    ...
    :param market_type: Type of market data to be imported
                        ('el' for electricity, 'gas' for natural gas, 'eua' for EUA prices).
...
def import_data(csvfile: str, path: str) -> np.ndarray:
    ...
    # Hydrogen reactant molar flow rate [mol/s]
    n_h2 = df["n_h2 [mol/s]"].values.astype(float)
    # Methane product molar flow rate [mol/s]
    n_ch4 = df["n_ch4 [mol/s]"].values.astype(float)
    # Hydrogen product molar flow rate (residues) [mol/s]
...
def load_data(
    ...
    dict_price_data = {
        f'{data[0]}_{split}': import_market_data(
            getattr(env_config, f'datafile_path_{split}_{data[1]}'), data[1], path
            )
    ...
    def check_market_data_size(split):
        ...
        if len({el_size_d, gas_size_d, eua_size_d}) > 1:
            warnings.warn(
                f"Market data size does not match for {split}: "
                f"electricity ({el_size_h}h = {el_size_d}d), "
    ...
    op_data_files = [
        ('startup_cold', 2), ('startup_hot', 3), ('cooldown', 4),
        ('standby_down', 5), ('standby_up', 6), ('op1_start_p', 7),
        ('op2_start_f', 8), ('op3_p_f', 9), ('op4_p_f_p_5', 10),
        ('op5_p_f_p_10', 11), ('op6_p_f_p_15', 12), ('op7_p_f_p_22', 13),
        ('op8_f_p', 14), ('op9_f_p_f_5', 15), ('op10_f_p_f_10', 16),
        ('op11_f_p_f_15', 17), ('op12_f_p_f_20', 18)
    ]
    dict_op_data = {
        key: import_data(getattr(env_config, f'datafile_path{num}'), path)
        for key, num in op_data_files
    }

    if env_config.scenario in [2, 3]:
        gas_price = env_config.ch4_price_fix if env_config.scenario == 2 else 0
        for key in ['gas_price_train', 'gas_price_val', 'gas_price_test']:
            dict_price_data[key] = np.full(len(dict_price_data[key]), gas_price)

        if env_config.scenario == 3:
    ...
    dict_price_data.update({f'{key}_reward_level': env_config.r_0_values[key]
    ...
    # RL training excludes 'min_train_len' days to maintain space
    # for the day-ahead market price forecast.
    env_config.train_len_d = len(dict_price_data['gas_price_train']) - min_train_len
    assert env_config.train_len_d > 0, ('The training set size must be '+
                                        f'greater than {min_train_len} days')
    if env_config.train_len_d % env_config.eps_len_d != 0:
        # Find all possible divisors of env_config.train_len_d
        divisors = [
            i
            for i in range(1, env_config.train_len_d + 1)
            if env_config.train_len_d % i == 0
        ]
        assert False, (f'The training set size {env_config.train_len_d} '
                       'must be divisible by the episode length - data/config_env.yaml '
                       f'-> eps_len_d : {env_config.eps_len_d}; Possible divisors are: {divisors}')

    # Ensure that the training set is larger or at least equal to the defined episode length
    ep_cond = env_config.train_len_d >= env_config.eps_len_d
    assert ep_cond, (f'Training set size ({env_config.train_len_d}) '
                     'must be larger or at least equal to the defined '
                     f'episode length ({env_config.eps_len_d})!')
...
class Preprocessing():
    ...
    def __init__(
        ...
        :param env_config: Configuration settings for the environment.
        :param train_config: Configuration settings for training.
        """
        ...
        # Dictionary containing the potential reward [pot_rew...] and
        # the boolean load identifier [part_full_b...]
        self.dict_pot_r_b = None
        # Defines the reward penalty level based on electricity, synthetic
        # natural gas (SNG), and EUA price levels
        self.r_level = None
        ...
        #       No. of day-ahead values = env_config.price_ahead + env_config.price_past
        ...
        # Variables for splitting the training set into randomly selected
        # subsets for episodic learning
        ...
        # Overhead factor for self.eps_ind to account for randomness in process selection
        # during multiprocessing (Must be an integer)
        self.overhead_factor = 10
        assert isinstance(self.overhead_factor, int), (f"Episode overhead ({self.overhead_factor}) "
                                                       "must be an integer!")
        self.n_eps = None           # Episode length in seconds
        self.num_loops = None       # No. of loops over the full training set during training

        # For Multiprocessing: self.n_eps_loops is used for definition of different eps_ind
        # for different processes (MTCS_Q\env\ptg_gym_env.py for reference)
        self.n_eps_loops = None         # Total No. of episodes across the entire training process

        # Compute potential reward and load identifier
        self.preprocessing_rew()
        # Convert day-ahead datasets into NumPy arrays for calculations.
        self.preprocessing_array()
        # Define episodes and generate indices for random subset selection in training
    ...
    def preprocessing_rew(self) -> None:
        ...
        # The function calculate_optimum() was moved from Preprocessing() to
        # the separate rl_opt.py file for better clarity.
        print("---Calculate the theoretical optimum, the potential reward, and the load identifier")
        stats_dict_opt_train = calculate_optimum(self.dict_price_data['el_price_train'],
                                                 self.dict_price_data['gas_price_train'],
                                                self.dict_price_data['eua_price_train'],
                                                "Training", self.env_config.stats_names)
        stats_dict_opt_val = calculate_optimum(self.dict_price_data['el_price_val'],
                                               self.dict_price_data['gas_price_val'],
                                                self.dict_price_data['eua_price_val'],
                                                "Validation", self.env_config.stats_names)
        stats_dict_opt_test = calculate_optimum(self.dict_price_data['el_price_test'],
                                                self.dict_price_data['gas_price_test'],
                                                self.dict_price_data['eua_price_test'],
                                                "Test", self.env_config.stats_names)
        stats_dict_opt_level = calculate_optimum(self.dict_price_data['el_price_reward_level'],
                                                 self.dict_price_data['gas_price_reward_level'],
                                                self.dict_price_data['eua_price_reward_level'],
                                                "reward_Level", self.env_config.stats_names)

        # Store datasets containing future values of potential rewards at two different load levels,
        # Additionally, store datasets for the load identifier
        # corresponding to future potential rewards.
    ...
    def preprocessing_array(self) -> None:
        ...
        total_len = self.env_config.price_ahead + self.env_config.price_past

        # e_r_b: Multi-Dimensional array which stores Day-ahead electricity price data as
        # well as Day-ahead potential rewards and load identifiers for the entire training
        # and test sets e.g. e_r_b_train[0, 12, 156] represents the current value of the
        # electricity price [0,-,-] at the 156ths entry of the electricity price data set
        self.e_r_b_train = np.zeros((3, total_len,
                                     self.dict_price_data['el_price_train'].shape[0] - total_len))
        self.e_r_b_val = np.zeros((3, total_len,
                                   self.dict_price_data['el_price_val'].shape[0] - total_len))
        self.e_r_b_test = np.zeros((3, total_len,
                                    self.dict_price_data['el_price_test'].shape[0] - total_len))
        ...
        # g_e: Multi-Dimensional Array which stores Day-ahead gas and EUA price data for
        # the entire training and test set
    ...
    def define_episodes(self) -> None:
        ...
        # Divide the full training dataset into smaller subsets,
        # each representing a separate episode
        # n_eps: No. of training subsets (episodes)
        self.n_eps = int(self.env_config.train_len_d / self.env_config.eps_len_d)
        self.eps_len = 24 * 3600 * self.env_config.eps_len_d  # Episode length in [s]

        # No. of steps per episode in training, validation, and testing
        self.eps_sim_steps_train = int(self.eps_len / self.env_config.sim_step)
        self.eps_sim_steps_val = int(24 * 3600 * val_len_d / self.env_config.sim_step)
        self.eps_sim_steps_test = int(24 * 3600 * test_len_d /self.env_config.sim_step)

        # Define the total number of loops over the entire training set for all workers
        self.num_loops = self.train_config.train_steps / (self.eps_sim_steps_train * self.n_eps)
        # Print training process details
        print("    > Total number of training steps =", self.train_config.train_steps)
        ...
        # Allows unique eps_ind assignments per process (MTCS_Q\env\ptg_gym_env.py for reference)
    ...
    def rand_eps_ind(self):
        ...
        2. Divide the training set into smaller subsets (train_len_d > eps_len_d),
           selecting subsets randomly.
        """
        # Set the random seed for random episode selection
        np.random.seed(self.train_config.seed_train)

        if self.env_config.train_len_d == self.env_config.eps_len_d:
            self.eps_ind = np.zeros(self.n_eps*int(self.num_loops)*self.overhead_factor)
        else:           # self.env_config.train_len_d > self.env_config.eps_len_d:
            # Random selection with replacement
            num_ep = np.linspace(start=0, stop=self.n_eps-1, num=self.n_eps)
            if self.num_loops < 1:
                num_loops_int = 1
            else:
                num_loops_int = int(self.num_loops)
            ...
            self.eps_ind = random_ep.reshape(int(self.n_eps *
                                                 num_loops_int *
    ...
    def dict_env_kwargs(self, train_val_test_type: str = "train") -> dict[str, Any]:
        ...
        :param train_val_test_type: Specifies whether the dataset is for training ("train") or
                                    for validation/testing ("val_test").
        :return: env_kwargs - Dictionary containing global parameters and hyperparameters.
        """

        # General environment configurations
        env_kwargs = {
            **{f"ptg_{key}": self.env_config.ptg_state_space[key] for key in
            ["standby", "cooldown", "startup", "partial_load", "full_load"]},
            **{key: getattr(self.env_config, key) for key in
                [
                "noise", "eps_len_d", "sim_step", "time_step_op", "price_ahead", "price_past",
                "scenario", "convert_mol_to_Nm3", "H_u_CH4", "H_u_H2", "dt_water", "cp_water",
                "rho_water", "Molar_mass_CO2", "Molar_mass_H2O", "h_H2O_evap", "eeg_el_price",
                "heat_price", "o2_price", "water_price", "min_load_electrolyzer",
                "max_h2_volumeflow", "eta_CHP", "t_cat_standby", "t_cat_startup_cold",
                "t_cat_startup_hot", "time1_start_p_f", "time2_start_f_p", "time_p_f", "time_f_p",
                "time1_p_f_p", "time2_p_f_p", "time23_p_f_p", "time3_p_f_p", "time34_p_f_p",
                "time4_p_f_p", "time45_p_f_p", "time5_p_f_p", "time1_f_p_f", "time2_f_p_f",
                "time23_f_p_f", "time3_f_p_f", "time34_f_p_f", "time4_f_p_f", "time45_f_p_f",
                "time5_f_p_f", "i_fully_developed", "j_fully_developed", "el_l_b", "el_u_b",
                "gas_l_b", "gas_u_b", "eua_l_b", "eua_u_b", "T_l_b", "T_u_b", "h2_l_b", "h2_u_b",
                "ch4_l_b", "ch4_u_b", "h2_res_l_b", "h2_res_u_b", "h2o_l_b", "h2o_u_b", "heat_l_b",
                "heat_u_b"
                ]
            },
        ...
        env_kwargs.update({
                # Length of the sequence for time-series encoding
                "seq_length": self.train_config.seq_length,
                # Number of data steps in process data between two consequitive entries
                # in the sequences
                "seq_step": self.train_config.seq_step,
                "state_change_penalty": self.env_config.state_change_penalty,
...
def initial_print() -> None:
    ...
    print('\n---------------------------------------MCTS_Q----------------------------------------')
    print('--Monte Carlo Tree Search and Deep Q Network for Power-to-Gas dispatch optimization--')
    print('-------------------------------------------------------------------------------------\n')

def config_print(
        env_config: EnvConfiguration,
        train_config: TrainConfiguration,
        mcts_config: MCTSQConfiguration
    ) -> str:
    """
    Gathers and prints general settings
    :param env_config: Environment configuration (class object)
    :param train_config: Training configuration (class object)
    :param mcts_config: MCTS_Q configuration (class object)
    :return str_id: String for identification of the present training run
    """
    print("Set training case...")
    print(f"---Training case details: MCTS_Q_{train_config.str_inv} ")
    str_id = "MCTS_Q_" + train_config.str_inv
    if env_config.scenario == 1:
        print("    > Business case (_BS):\t\t\t 1 - Trading at the"
              " electricity, gas, and emission spot markets")
    elif env_config.scenario == 2:
        print("    > Business case  (_BS):\t\t\t 2 - Fixed synthetic"
              " natural gas (SNG) price and trading at the electricity"
              " and emission spot markets")
    else:
        print("    > Business case (_BS):\t\t\t 3 - Participating in EEG tenders"
              " by using a CHP plant and trading at the electricity spot markets")
    str_id += "_BS" + str(env_config.scenario)
    print(f"    > Operational load level (_OP) :\t\t {env_config.operation}")
    str_id += "_" + str(env_config.operation)
    print(f"    > Training episode length (_ep) :\t\t {env_config.eps_len_d} days")
    str_id += "_ep" + str(env_config.eps_len_d)
    print(f"    > Time step size (action frequency) (_ts) :\t {env_config.sim_step} seconds")
    str_id += "_ts" + str(env_config.sim_step)
    print(f"    > Random seed (_rs) :\t\t\t {train_config.seed_train}")
    str_id += mcts_config.get_hyper()
    # Random seed at the end of "str_id" for file simplified file
    str_id += "_rs" + str(train_config.seed_train)

    return str_id

class CallbackVal():
    """ A class for validation callbacks during training """
...
def create_envs(
        ...
        train_config: TrainConfiguration
    ) -> tuple[gym.Env, CallbackVal, gym.Env]:
    """Creates environments for training, validation, and testing"""

    env_train = gym.make(
        env_id,
        dict_input=env_kwargs_data['env_kwargs_train'],
        train_or_eval="train"
    )
    env_test_post = gym.make(
        env_id,
        dict_input=env_kwargs_data['env_kwargs_test'],
        train_or_eval="eval"
    )

    callback_val = CallbackVal(
        env_id,
        env_kwargs_data['env_kwargs_val'],
...
def plot_results(
        env_config: EnvConfiguration,
        stats_dict_test: dict[str, Any],
        str_id: str
    ) -> None:
    """
    Generates a multi-subplot plot displaying time-series data and methanation
    operations based on the agent's actions
    """
    print("---Plot and save RL performance on the test set under ./plots/ ...\n")

    stats_dict = stats_dict_test
    time_sim = stats_dict['steps_stats'] * env_config.sim_step
    # Converts the simulation time into days
    time_sim *= 1 / 3600 / 24
    # ptg_gym_env.py curtails an episode by 6 time steps to ensure a data overhead
    time_sim = time_sim[:-6]
    ...
    axs1_1.plot(
        time_sim,
        stats_dict['Meth_CH4_flow_stats'][:-6]*1000,
        'yellowgreen',
        label='CH$_4$'
    )
    ...
    axs[2].plot(
        time_sim,
        stats_dict['Meth_reward_stats'][:-6]/100,
        'g',
        label='Reward'
    )
    ...
    fig.suptitle(
        f"{str_id} \n Rew: {np.round(stats_dict['Meth_cum_reward_stats'][-7]/100, 0)} €",
        fontsize=9
    )
